(function ($) {
    $.fn.simpleChart = function (options) {
		...
		// extend true will merge options onto defaults
		$.extend( true, config, options );
		
        var template, leftPosition = [], chartType, chartHeight, chartWidth, barColor, barMargin, i, titleAlign, labelInterval,
        	prefix = '', suffix = '';
		...
		if(typeof config.item.outputValue == 'undefined') config.item.outputValue = config.item.value.slice(0);
		labelInterval = config.item.labelInterval;
		prefix = config.item.prefix;
		suffix = config.item.suffix;
        ...
        var tooltips = (typeof config.item.tooltip === 'undefined')? config.item.value : config.item.tooltip;
        ...
        if (config.type == 'bar') {
            ...
            for (i = 0; i < config.item.value.length; i++) {
                ...
                template += '<span class="sc-value">' + prefix + (config.item.outputValue[i] || config.item.value[i]) + suffix + '</span>';
                template += '<div class="sc-tooltip">';
                // template += '<span class="sc-tooltip-value">' + prefix + (config.item.outputValue[i] || config.item.value[i]) + suffix + '</span>';
                template += '<span class="sc-tooltip-value">' + prefix + tooltips[i] + suffix + '</span>';
        ...
        if (config.type == 'column') {
            ...
            for (i = 0; i < config.item.value.length; i++) {
                barColor = getBackgroundColor();
                template += '<div class="sc-item" style="left:' + ((itemWidth + barMargin) * i + barMargin) + '%;width:' + (itemWidth) + 
                	'%;height:' + itemHeight[i] + '%;' + barColor + '">';
                template += '<span class="sc-label">' + ((i % labelInterval == 0)? config.item.label[i] : '') + '</span>';
                template += '<span class="sc-value">' + prefix + (config.item.outputValue[i] || config.item.value[i]) + suffix + '</span>';
                template += '<div class="sc-tooltip">';
                // template += '<span class="sc-tooltip-value">' + prefix + (config.item.outputValue[i] || config.item.value[i]) + suffix + '</span>';
                template += '<span class="sc-tooltip-value">' + prefix + tooltips[i] + suffix + '</span>';
        ...
        if (config.type == 'step') {
            ...
            for (i = 0; i < config.item.value.length; i++) {
                ...
                template += '<div class="sc-item' + stepClass + '" style="left:' + ((itemWidth + barMargin) * i) + '%;width:' + (itemWidth) + 
                	'%;top:' + (100 - itemPercentage[i]) + '%;' + barColor + '">';
                template += '<span class="sc-label">' + config.item.label[i] + '</span>';
                template += '<span class="sc-value">' + prefix + (config.item.outputValue[i] || config.item.value[i]) + suffix + '</span>';
                template += '<div class="sc-tooltip">';
                // template += '<span class="sc-tooltip-value">' + prefix + (config.item.outputValue[i] || config.item.value[i]) + suffix + '</span>';
        ...
        if (config.type == 'progress') {
            ...
            for (i = 0; i < config.item.value.length; i++) {
                ...
                template += '<div class="sc-item" style="left:' + leftPosition[i] + '%;width:' + itemWidth[i] + '%;' + barColor + 
                	'z-index:' + (config.item.value.length - i) + '">';
                template += '<span class="sc-label">' + config.item.label[i] + '</span>';
                template += '<span class="sc-value">' + prefix + (config.item.outputValue[i] || config.item.value[i]) + suffix + '</span>';
                template += '<div class="sc-tooltip">';
                // template += '<span class="sc-tooltip-value">' + prefix + (config.item.outputValue[i] || config.item.value[i]) + suffix + '</span>';
